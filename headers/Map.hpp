...
class Map 
{
    template <typename Key1, typename Data1> friend std::ostream& operator<<(std::ostream& out, const Map<Key1, Data1>& rhv);
private:
    struct Node {
        Node(const Key& key,
             const Data& data,
...
public:
    ...
    ~Map();
    const Map& operator=(const Map& rhv);
    void swap(Map& rhv);
    ...
    bool empty() const; 
    bool operator==(const Map& rhv) const;
    bool operator!=(const Map& rhv) const;
    bool operator< (const Map& rhv) const;
    bool operator<=(const Map& rhv) const;
    bool operator> (const Map& rhv) const;
...
private:
    ...
    void insertMapHelper(Node* root);
